...
const FileUploadConvertToDocx = () => {
    const [hovering, setHovering] = useState(false);
    const [message, setMessage] = useState('');
    const [selectedFile, setSelectedFile] = useState(null);
    const [fileList, setFileList] = useState([]); // Store the list of files
    const [buttonText, setButtonText] = useState('Upload and Convert to DOCX'); // Button text state
    ...
    const handleFileChange = (event) => {
        ...
        if (files.length > 0) {
            ...
            if (existingFile) {
                ...
                setButtonText('Convert to DOCX');
                setMessage(`File already exists: ${file.name}. You can convert it to DOCX.`);
            } else {
                setSelectedFile(file);
                setIsFileExisting(false);
                setButtonText('Upload and Convert to DOCX');
                setMessage(`Selected file: ${file.name}`);
            }
        }
    };

    const handleFileUploadOrConvertToDocx = async () => {
        if (!selectedFile) {
        ...
        try {
            if (!isFileExisting) {
                ...
                setMessage('File uploaded successfully! Now converting to DOCX...');
            }

            // Convert PDF to DOCX
            const response = await axios.post(
                `http://103.145.63.232:8081/api/pdf/convertToDocx?fileName=${encodeURIComponent(fileName)}`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.status === 200) {
                setMessage('PDF converted to DOCX successfully!');
                setTimeout(() => {
                    navigate('/storage');
            ...
            setMessage(`There was an error converting the PDF to DOCX: ${error.response?.data?.message || error.message}`);
        }
    };
    ...
    return (
        ...
        >
            ...
            <p className="mt-4 text-center">Select a <strong>PDF</strong> file to convert to DOCX</p>
            ...
            <select
                onChange={(e) => {
                    ...
                    setButtonText('Convert to DOCX'); // Change button text to "Convert to DOCX"
            ...
            <button
                onClick={handleFileUploadOrConvertToDocx}
...
export default FileUploadConvertToDocx;
