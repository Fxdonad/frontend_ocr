...
const FileUploadConvertToXlsx = () => {
    const [hovering, setHovering] = useState(false);
    const [message, setMessage] = useState('');
    const [selectedFile, setSelectedFile] = useState(null);
    const [fileList, setFileList] = useState([]); // Store the list of files
    const [buttonText, setButtonText] = useState('Upload and Convert to XLSX'); // Button text state
    ...
    const handleFileChange = (event) => {
        ...
        if (files.length > 0) {
            ...
            if (existingFile) {
                ...
                setButtonText('Convert to XLSX');
                setMessage(`File already exists: ${file.name}. You can convert it to XLSX.`);
            } else {
                setSelectedFile(file);
                setIsFileExisting(false);
                setButtonText('Upload and Convert to XLSX');
                setMessage(`Selected file: ${file.name}`);
            }
        }
    };

    const handleFileUploadOrConvertToXlsx = async () => {
        if (!selectedFile) {
        ...
        try {
            if (!isFileExisting) {
                ...
                setMessage('File uploaded successfully! Now converting to XLSX...');
            }

            // Convert PDF to XLSX
            const response = await axios.post(
                `http://103.145.63.232:8081/api/pdf/convertToXlsx?fileName=${encodeURIComponent(fileName)}`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.status === 200) {
                setMessage('PDF converted to XLSX successfully!');
                setTimeout(() => {
                    navigate('/storage');
            ...
            setMessage(`There was an error converting the PDF to XLSX: ${error.response?.data?.message || error.message}`);
        }
    };
    ...
    return (
        ...
        >
            ...
            <p className="mt-4 text-center">Select a <strong>PDF</strong> file to convert to XLSX</p>
            ...
            <select
                onChange={(e) => {
                    ...
                    setButtonText('Convert to XLSX'); // Change button text to "Convert to XLSX"
            ...
            <button
                onClick={handleFileUploadOrConvertToXlsx}
...
export default FileUploadConvertToXlsx;
